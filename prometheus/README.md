...
helm install prometheus ar-grafana/prometheus -n monitor
```
...
```console
$ helm install ar-grafana/prometheus prometheus \
    --set server.terminationGracePeriodSeconds=360 -n monitor
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart. For example,

```console
$ helm install ar-grafana/prometheus prometheus -f values.yaml -n monitor
```
...
```console
$ helm install ar-grafana/prometheus prometheus -f values.yaml -f service1-alert.yaml -f service2-alert.yaml -n monitor
```
...
```console
kubectl create secret tls prometheus-server-tls --cert=path/to/tls.cert --key=path/to/tls.key -n monitor
```
